...
export default function ApiDocs() {
  ...
  "total": 128
    ...
    // ----------------------------------------
    {
      ...
      path: '/api/stats',
      method: 'GET',
      description: '返回图片统计信息（各文件夹数量、外部图片数量、总数）',
  ...
  "grand_total": 71
    ...
    // ----------------------------------------
    {
      ...
      path: '/api/list',
      method: 'GET',
      description: '返回所有图片列表（按文件夹分组）',
      example: `curl ${baseUrl}/api/list`,
      response: '返回 JSON 格式的图片列表，包含文件名、URL、大小等信息'
    },
    
    // ----------------------------------------
    // 7. 上传图片接口
    // POST 请求，需要 multipart/form-data 格式
    // 参数：file（图片文件）、folder（目标文件夹）
    ...
    // ----------------------------------------
    {
      ...
      path: '/api/upload',
      method: 'POST',
      description: '上传图片到指定分类（支持 JPG/PNG/WebP/GIF/AVIF，最大 25MB）',
  ...
  "url": "${baseUrl}/api/image?path=wallpaper/20260616_image.jpg"
    ...
    // ----------------------------------------
    {
      ...
      path: '/api/image',
      method: 'GET',
      description: '代理访问私有仓库中的图片（解决 GitHub 私有仓库无法直接访问的问题）',
      example: `${baseUrl}/api/image?path=wallpaper/20260616_image.jpg`,
      response: '直接返回图片文件'
    }
  ]
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mt-8 text-gray-400 dark:text-gray-500 text-xs">
          ...
          <p className="mt-1">
            更多信息请访问{' '}
            <a 
              href="https://github.com/chnbsdan/Pico" 
              target="_blank" 
              rel="noopener noreferrer" 
