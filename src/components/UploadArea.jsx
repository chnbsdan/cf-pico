...
export default function UploadArea({ onUpload, isLoading, convertToWebp, onConvertChange }) {
  ...
  // 修改点 8：HuggingFace 上传时设置 setIsNormalUploading 让进度条显示
  // ============================================================
    ...
    for (const file of fileArray) {
      ...
      try {
        ...
        if (needChunk) {
          ...
          // GitHub 现在支持大文件（走 Releases），只限制 R2
              if (actualStorage === 'r2' && file.size > 25 * 1024 * 1024) {
                throw new Error(`R2 限制 25MB，请切换到 Telegram 或 HuggingFace`)
}
