...
| `GITHUB_REPO` | 纯文本 | ❌ | `chnbsdan/cf-pico` | **GitHub 仓库完整名称（格式：`用户名/仓库名`）** |
| `GITHUB_RELEASE_TAG` | 纯文本 | ❌ | `cf-pico-storage` | GitHub Release 标签名（大文件存储） |
| `PUBLIC_BASE_URL` | 纯文本 | ❌ | 无 | **公网完整域名（格式：`https://你的域名.com`）** |
...
| `/api/hf/{path}` | GET | HuggingFace 文件代理 |
| `/api/github?path={filename}` | GET | GitHub Release 文件代理（支持在线预览） |
...
| `GITHUB_REPO` | ❌ | `chnbsdan/cf-pico` | **GitHub 仓库完整名称（格式：`用户名/仓库名`）** |
| `GITHUB_RELEASE_TAG` | ❌ | `cf-pico-storage` | GitHub Release 标签名（用于大文件存储） |
| `PUBLIC_BASE_URL` | ❌ | 无 | **公网完整域名（格式：`https://你的域名.com`）** |
